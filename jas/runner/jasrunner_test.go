...
func TestJasRunner(t *testing.T) {
	...
	testParams := JasRunnerParams{
		...
		CvesProvider: func() (directCves []string, indirectCves []string) {
			return applicability.ExtractDependenciesCvesFromScan(targetResults.GetScaScansXrayResults(), directComponents)
		},
	}
...
func TestJasRunner_AnalyzerManagerReturnsError(t *testing.T) {
	...
	directCves, indirectCves := applicability.ExtractDependenciesCvesFromScan(jas.FakeBasicXrayResults, []string{"issueId_2_direct_dependency", "issueId_1_direct_dependency"})
