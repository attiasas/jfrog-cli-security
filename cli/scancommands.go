...
func DockerScanMockCommand() components.Command {
	...
	return components.Command{
		...
		Action: func(c *components.Context) error {
			...
			if show, err := cliutils.ShowGenericCmdHelpIfNeeded(c.Arguments, func() error { return c.PrintCommandHelp(dockerScanCmdHiddenName) }); show || err != nil {
				return err
			}
...
func DockerScan(c *components.Context, image string) error {
	if image == "" {
		return pluginsCommon.PrintHelpAndReturnError("a docker image name must be provided", c)
	}
	threads, err := pluginsCommon.GetThreadsCount(c)
	if err != nil {
		return err
