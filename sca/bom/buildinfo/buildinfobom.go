...
type BuildInfoBomParams struct {
...
type BuildInfoBomGenerator struct {
	Params BuildInfoBomParams
}

func (bbg *BuildInfoBomGenerator) Parallel(threadId int) bom.SbomGenerator {
	return bbg
}

func (bbg *BuildInfoBomGenerator) GenerateSbom(target results.ScanTarget) (sbom *cyclonedx.BOM, err error) {
	...
	treeResult, bdtErr := buildDependencyTree(target, bbg.Params)
	if bdtErr != nil {
...
func buildDependencyTree(scan results.ScanTarget, params BuildInfoBomParams) (*DependencyTreeResult, error) {
...
func SetResolutionRepoInParamsIfExists(params BuildInfoBomParams, tech techutils.Technology) (serverDetails *config.ServerDetails, err error) {
	serverDetails = params.ServerDetails
	if params.DependencyRepository != "" || params.IgnoreConfigFile {
	...
	params.DependencyRepository = artifactoryDetails.TargetRepository
	params.ServerDetails = artifactoryDetails.ServerDetails
	serverDetails = artifactoryDetails.ServerDetails
	return
}

func GetTechDependencyTree(params BuildInfoBomParams, artifactoryServerDetails *config.ServerDetails, tech techutils.Technology) (depTreeResult DependencyTreeResult, err error) {
	...
	case techutils.Maven, techutils.Gradle:
		depTreeResult.FullDepTrees, uniqDepsWithTypes, err = java.BuildDependencyTree(java.DepTreeParams{
			...
			DepsRepo:                params.DependencyRepository,
...
func getCurationCacheFolderAndLogMsg(params BuildInfoBomParams, tech techutils.Technology) (logMessage string, curationCacheFolder string, err error) {
